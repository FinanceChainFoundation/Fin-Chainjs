...
function _broadcast(was_broadcast_callback){
    return new Promise((resolve, reject)=> {
        ...
        if(this.broadcast_method=="broadcast_transaction_with_callback")
            Apis.instance().network_api().exec( this.broadcast_method, [ function(res) { return resolve(res); } ,tr_object]).then(function(){
            ...
            Apis.instance().network_api().exec( this.broadcast_method,tr_object)
        return;
    });
}
