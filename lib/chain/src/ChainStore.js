...
let lock_balance_prefix = "2." + parseInt(impl_object_type.lock_balance,10) + ".";
    ...
    fetchFullAccount( name_or_id, autosubscribe = true )
    {
        ...
        if( subChanged || !this.fetching_get_full_accounts.has(name_or_id) || (Date.now() - this.fetching_get_full_accounts.get(name_or_id)) > 5000  ) {
            ...
            .then( results => {
                ...
                for( let id in full_account.fix_balances){
                    full_account.fix_balances[id].forEach(fix_balance=>{
                        this._updateObject(fix_balance);
                    })
                }
