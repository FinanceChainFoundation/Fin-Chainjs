...
let lock_balance_prefix = "2." + parseInt(object_type.lock_balance,10) + ".";
let asset_lock_data_prefix="2." + parseInt(impl_object_type.asset_lock_data,10) + ".";
...
class ChainStore
{
    ...
    fetchFullAccount( name_or_id, autosubscribe = true )
    {
        ...
        if( subChanged || !this.fetching_get_full_accounts.has(name_or_id) || (Date.now() - this.fetching_get_full_accounts.get(name_or_id)) > 5000  ) {
            ...
            .then( results => {
                ...
                account.balances = {};
                account.fix_balances = Immutable.fromJS(full_account.fix_balances);
                ...
                account.presales = new Immutable.Set();
                let sub_to_objects = [];
                ...
                if (full_account.presales)
                    account.presales = account.presales.withMutations(set => {
                        presales.forEach(p =>  {
                            this._updateObject(p);
                            set.add(p);
    ...
    */
    fetchRecentHistory( account, limit = 20 )
    {
        ...
        if (history && history.size) {
            const ids = history.toJS().map(i => Number(i.id.substr(i.id.lastIndexOf('.') + 1)));
            ids.sort((a, b) => {
                if (a > b) {
                    return 1;
                }else if(a < b){
        ...
        */
        ...
        if( object.id.substring(0,balance_prefix.length) == balance_prefix ) {
            ...
            if( owner === undefined || owner === null ) {
                ...
                let fix_balances = owner.get("fix_balances");
                if (!fix_balances) {
                    owner = owner.set("fix_balances", Immutable.Map());
                    owner = owner.setIn(["fix_balances", object.asset_id], object.id);
                }
                this.objects_by_id.set(object.owner, owner);
            }}
        else if( object.id.substring(0,account_stats_prefix.length) == account_stats_prefix ) {
            ...
            Apis.instance().db_api().exec("get_asset_presales", [object.id]).then(function (presales_object){
                ...
                if(presales_object.length>0)
                    current = current.set("presales",presales_object)
